...
async def handle_new_member(message: Message, bot):
    if str(message.chat.id) != CHAT_ID:
        error_logger.warning(f"Бот использован в неразрешенном чате: {message.chat.id}")
        return
    ...
    try:
        ...
        for member in message.new_chat_members:
            user_mention = f"@{member.username}" if member.username else f"{member.first_name or 'Участник'}"

            # Приветствие в чате + кнопка
            welcome_msg = (
                f"{user_mention}, Привет! 💜\n"
                "Мы рады, что ты с нами. Этот чат создан для поддержки, обмена опытом и добрых разговоров. "
                "Чтобы тебе было легче освоиться, загляни в ветку "
                '<a href="https://t.me/c/1813228738/65079">Важное и правила</a> — там вся основная информация для комфортного общения.\n'
                ...
                "Здесь тебя выслушают и поддержат. Добро пожаловать! 🌟\n\n"
            ...
            asyncio.create_task(
                delete_message_after_delay(bot, sent_message.chat.id, sent_message.message_id)
            )
